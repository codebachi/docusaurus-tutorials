...
거의 모든 VCS<sup>버전 관리 시스템</sup>는 어떤 형식이든 브랜칭과 유사한 기능을 지원합니다. 브랜칭이란 개발중인 메인 라인에서 마치 나무의 가지처럼 갈라져 나와 메인 개발 라인을 개판으로 만들 염려 없이 일을 진행할 수 있다는 얘기입니다. 개발을 하다 보면 코드를 여러 개로 복사해야 하는 일이 자주 생깁니다. 코드를 통째로 복사하고 나서 원래 코드와는 상관없이 독립적으로 개발을 진행할 수 있는데, 이렇게 독립적으로 개발하는 것이 브랜치다.
